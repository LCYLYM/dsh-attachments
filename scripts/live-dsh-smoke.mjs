...
const pluginRoot=process.argv[4]?path.resolve(process.argv[4]):root;
...
await fs.writeFile(path.join(profile,'package.json'),JSON.stringify({name:'ba-live-profile',private:true,dependencies:{'dsh-multimedia-webui-input':'file:'+pluginRoot},dsh:{profile:{bundles:['@deepseek-ai/dsh-base','@deepseek-ai/dsh-web-app','dsh-multimedia-webui-input']}}}));
await fs.mkdir(path.join(profile,'node_modules'),{recursive:true});await fs.symlink(pluginRoot,path.join(profile,'node_modules/dsh-multimedia-webui-input'),'junction');
