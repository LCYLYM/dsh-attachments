...
const temp=await fs.mkdtemp(path.join(os.tmpdir(),'ba-live-')),result=path.join(temp,'result.json');
...
const child=spawn(process.execPath,[path.resolve(cli),'web','--no-open'],{env:{...process.env,DSH_HOME:home},stdio:['ignore','pipe','pipe']});let logs='';child.stdout.on('data',b=>logs+=b);child.stderr.on('data',b=>logs+=b);
let report;
