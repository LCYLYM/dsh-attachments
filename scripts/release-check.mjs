...
try{live=JSON.parse(await fs.readFile(path.join(root,'artifacts/live-dsh.json'),'utf8'));}catch{failures.push('No artifacts/live-dsh.json. Fill the template only after real DSH tests.');}
if(live){
...
const report={version:'0.2.0-rc.1',publicReleaseReady:!failures.length,reason:failures,scope:'manual native-runtime release gate; not a cryptographic attestation'};
