...
export async function apply(ctx,config){
 ...
 try{
  ...
  await run('workspace upload requests are rejected without creating a workspace copy',async()=>{
   const res=await fetch(api+'/batches',{method:'POST',headers:{'x-better-attach':'1','x-ba-session':'@workspace-import','content-type':'application/json'},body:JSON.stringify({id:randomUUID(),sessionId:'@workspace-import',target:'workspace',files:[],directories:['folder']})});assert.equal(res.status,403);assert.equal((await res.json()).error.code,'workspace-scope');
   await assert.rejects(fs.stat(path.join(config.root,'storage','workspaces')),{code:'ENOENT'});
  });
