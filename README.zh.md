...
版本 **0.3.0-rc.5**。需要 Node.js `^22.19.0 || >=24.0.0` 和 DSH。当前验证的 CLI 为 `0.1.5-rc.1`（latest）、`0.1.5-rc.2`（next）；完整组件版本由 fixtures 的锁文件固定。
...
需要隔离试用时，先设置 `DSH_HOME` 指向一个新目录，再执行安装和启动命令。插件的附件记录保存在该目录下的 `better-attach/`；会话附件副本位于当前工作区的 `.dsh/tmp/attachments/better-attach-v2/`。
...
| 对话附件模式 | 添加方式 | 原文件后续变化 |
| --- | --- | --- |
| 复制副本（默认） | 检查目录与预览，暂存成一张卡片，发送时复制 | 已保存副本保持独立 |
| 引用路径 | 填写主机路径，校验后添加引用 | 模型读取当前原文件 |

**侧栏始终添加主机原目录为工作区。** 拖放数据含原绝对路径时直接注册；浏览器未提供时，选择或填写 DSH 主机上的原目录。

在 **设置 → Better Attach** 或输入区的 **附件设置** 中切换对话附件模式。普通浏览器通常不提供原绝对路径；对话引用模式需要明确填写运行 DSH 的主机路径。
