...
| 拖进侧栏 | 添加主机原目录为工作区 | 添加主机原目录为工作区 |
| 原文件后续变化 | 已保存副本保持独立 | 模型读取当前原文件 |

侧栏独立于对话附件模式。拖放数据含原绝对路径时直接注册；浏览器未提供时，选择或填写 DSH 主机上的原目录。

在 **设置 → Better Attach** 或输入区的 **附件设置** 中切换模式。普通浏览器通常不提供原绝对路径；对话引用模式需要明确填写运行 DSH 的主机路径。
