import { PathReferences } from './references.js';
...
export function createHandler(store, { nativeDrop } = {}) {
  const references = new PathReferences(store);
  return async (req, res) => {
    try {
      ...
      if (rest.startsWith('/native-drop')) {
        assert(session === '@workspace-import', 'workspace-scope', 'Use the dedicated workspace scope', 403);
        if (req.method === 'GET' && rest === '/native-drop') return reply(res, 200, { ok: true, ...(nativeDrop?.status() ?? { available: false, state: 'unavailable' }) });
        assert(nativeDrop, 'native-unavailable', 'Native workspace receiver is unavailable', 503);
        if (req.method === 'POST' && rest === '/native-drop') return reply(res, 200, { ok: true, ...nativeDrop.arm((await jsonBody(req)).language) });
        const ticket = /^\/native-drop\/([a-f0-9-]{36})$/.exec(rest);
        assert(ticket, 'route', 'Unknown native workspace route', 404);
        if (req.method === 'DELETE') { nativeDrop.cancel(ticket[1]); return reply(res, 200, { ok: true }); }
        if (req.method === 'POST') { nativeDrop.resolve(ticket[1], (await jsonBody(req)).name); return reply(res, 200, { ok: true }); }
        if (req.method === 'GET') {
          const controller = new AbortController(), closed = () => controller.abort();
          res.on('close', closed);
          try { const value = await nativeDrop.consume(ticket[1], controller.signal); if (!res.destroyed) reply(res, 200, { ok: true, ...value }); }
          finally { res.off('close', closed); }
          return;
        }
        assert(false, 'method', 'Method not allowed', 405);
      }
      ...
      if (req.method === 'GET' && rest === '/references') return reply(res,200,{ok:true,references:await references.list(session)});
      if (req.method === 'POST' && rest === '/references') return reply(res,200,{ok:true,reference:await references.create(session,(await jsonBody(req)).path)});
      const refMatch=/^\/references\/([a-f0-9-]{36})(\/file)?$/.exec(rest);
      if(req.method==='GET'&&refMatch){
        if(!refMatch[2])return reply(res,200,{ok:true,reference:await references.get(session,refMatch[1])});
        const {handle,record}=await references.open(session,refMatch[1]);
        res.writeHead(200,{'content-type':'application/octet-stream','content-disposition':"attachment; filename*=UTF-8''"+encodeURIComponent(record.name),'cache-control':'no-store','x-content-type-options':'nosniff'});
        try{await pipeline(handle.createReadStream(url.searchParams.get('preview')==='1'?{start:0,end:128*1024-1}:{}),res);}finally{await handle.close();}
        return;
      }
