...
import { captureDrop, workspaceDrop, isExternalFiles, pickList, readDrop, reviewSelection, selectionLabel } from './intake.js';
import { AttachClient } from './client-api.js';
import { nativeWorkspaceDrops } from './native-drop-client.js';
const messages = {
  zh: {
    files:'添加文件', folder:'添加文件夹', history:'附件记录', close:'关闭', cancel:'取消', retry:'重试', reattach:'再次附加', retryPrepare:'重试保存', existing:'使用主机原目录', existingHint:'输入运行 DSH 的主机上的绝对路径。这不会上传或复制文件，也不能读取浏览器没有提供的本机路径。', register:'添加原目录', remove:'移除', undo:'撤销', preview:'预览', download:'下载', search:'筛选文件…', empty:'尚无附件', ready:'发送时复制', uploading:'正在传输', committed:'已保存', error:'传输失败', cancelled:'已暂停', removed:'已移除附件', add:'添加到对话', import:'添加工作区', inspect:'检查导入内容', fileCount:'个文件', folderCount:'个目录', skipped:'已跳过', sensitive:'包含敏感文件（明确确认后才导入）', folderHint:'保留目录结构；只在发送时复制，不改动原文件。', workspaceHint:'工作区直接使用主机原目录。浏览器未提供原路径时，请选择或填写该目录。', copyHint:'这是路径附件，预览不等于模型已收到原生图片。单独拖入图片可使用 DSH 原生图像通道。', excludedHint:'默认忽略依赖、缓存和常见敏感文件。不等同于完整 .gitignore 或秘密扫描。', scanning:'正在读取目录…', dropChat:'添加到这段对话', dropWorkspace:'添加原目录为工作区', dropSub:'松开后检查内容，不会立即复制', importSuccess:'工作区已添加', openWorkspace:'打开工作区', path:'副本位置', imported:'导入成功', noSession:'请先打开一个可编辑的对话', noNative:'此选择为路径附件；原生图像请单独拖入输入框。', unavailable:'预览不可用', textLimit:'仅预览前 128 KiB；代码、HTML 和 SVG 只作为文本展示。', noPreview:'此格式不在浏览器内执行或解析。可以下载原文件。', imagesLimit:'图片超过 20 MiB，仅提供下载。', historyHint:'这是本插件已落盘的附件，不代表模型消息已成功发送。删除会使旧消息中的路径失效。', clear:'清理当前对话附件', confirmDelete:'删除已保存的附件？', deleteHint:'只删除 Better Attach v2 在当前对话创建的副本。原文件和已注册工作区不会被删除。旧版附件不在清理范围内。', confirm:'确认删除', cleanup:'附件副本已清理', remaining:'展开其余文件', browserMemory:'未发送选择只保留在此浏览器页面；刷新会丢失选择。', blocked:'当前输入已锁定，附件未添加。', readFailure:'目录读取失败', workspaceUnavailable:'当前 DSH 缺少工作区注册能力；不会假装添加成功。', archiveTitle:'已保存附件', noMatch:'没有匹配文件', duplicates:'这个选择已经在草稿中', cancelHint:'暂停保留已完成文件。重试继续，不会重复创建副本。', inspectFolder:'查看文件夹', preparing:'正在确认…', includeNote:'已跳过的依赖目录不会递归读取；敏感文件可单独确认。', workspaceDropHint:'松开后添加主机原目录',workspaceLocation:'原目录位置', inspectSkipped:'查看跳过项', continue:'继续', tooManyDrafts:'草稿附件总量过大，请先发送或移除部分附件。'
  },
  en: {
    files:'Attach files', folder:'Attach folder', history:'Attachments', close:'Close', cancel:'Cancel', retry:'Retry', reattach:'Attach again', retryPrepare:'Retry saving', existing:'Use existing host directory', existingHint:'Enter an absolute path on the machine running DSH. No files are uploaded or copied. This cannot reveal a browser-local original path.', register:'Add original directory', remove:'Remove', undo:'Undo', preview:'Preview', download:'Download', search:'Filter files…', empty:'No attachments yet', ready:'Copied on send', uploading:'Transferring', committed:'Saved', error:'Transfer failed', cancelled:'Paused', removed:'Attachment removed', add:'Add to conversation', import:'Add workspace', inspect:'Review import', fileCount:'files', folderCount:'directories', skipped:'Skipped', sensitive:'Include sensitive files (explicit consent)', folderHint:'Preserves the folder structure. Copies only when you send. Originals stay untouched.', workspaceHint:'Workspaces use the original host directory. Select or enter it when the browser does not provide the original path.', copyHint:'Path attachment: seeing a preview does not mean the model received a native image. Drop images separately to use DSH’s native image channel.', excludedHint:'Dependencies, caches and common sensitive names are excluded by default. This is not a full .gitignore parser or secret scanner.', scanning:'Reading directory…', dropChat:'Attach to this conversation', dropWorkspace:'Add original directory as workspace', dropSub:'Release to review. Nothing is copied yet.', importSuccess:'Workspace added', openWorkspace:'Open workspace', path:'Copy location', imported:'Import complete', noSession:'Open an editable conversation first', noNative:'This selection is a path attachment. Drop images separately for native image input.', unavailable:'Preview unavailable', textLimit:'Showing at most 128 KiB. Code, HTML and SVG are displayed only as text.', noPreview:'This format is not executed or parsed in the browser. Download the original file instead.', imagesLimit:'Images over 20 MiB are download-only.', historyHint:'These files were saved by this plugin. That is not proof a model message was sent. Deleting them breaks paths in earlier messages.', clear:'Clean this conversation’s attachments', confirmDelete:'Delete saved attachments?', deleteHint:'Only Better Attach v2 copies for this conversation are deleted. Originals, registered workspaces and legacy attachments are untouched.', confirm:'Confirm deletion', cleanup:'Attachment copies removed', remaining:'Show remaining files', browserMemory:'Unsent selections live in this browser page. Refreshing loses them.', blocked:'The composer is locked. Nothing was attached.', readFailure:'Could not read directory', workspaceUnavailable:'This DSH instance has no workspace registration capability. Nothing was registered.', archiveTitle:'Saved attachments', noMatch:'No matching files', duplicates:'This selection is already in the draft', cancelHint:'Pause retains completed files. Retry continues without creating a second copy.', inspectFolder:'Inspect folder', preparing:'Preparing…', includeNote:'Skipped dependency folders are not traversed. Sensitive files can be explicitly included.', workspaceDropHint:'Release to add the original host directory',workspaceLocation:'Original directory', inspectSkipped:'Inspect skipped entries', continue:'Continue', tooManyDrafts:'Too many draft attachments. Send or remove some first.'
  }
};
Object.assign(messages.zh,{settings:'附件设置',mode:'对话附件处理方式',copyMode:'复制副本',pathMode:'引用路径',modeHint:'复制模式在发送时保存副本；引用模式只引用主机原文件，后续编辑会影响读取内容。',theme:'外观',plain:'原生 · 无图',official:'官网风格 · 蓝色渐变',character:'DSH 娘 · 自定义图片',copyPath:'复制路径',pathCopied:'路径已复制',reference:'引用原路径',pathHint:'浏览器不能提供绝对路径。请填写 DSH 主机上的文件或目录路径；不上传、不复制。',pathLabel:'主机绝对路径',referenceReady:'引用原文件',addPath:'添加路径引用',saveFailed:'设置保存失败',nativeHint:'独立图片使用 DSH 原生视觉附件。文件夹中的图片保留目录结构，作为路径读取。',themeHint:'背景只用于附件面板，不遮挡对话正文；切回无图立即还原。',hostPicker:'选择主机目录',pathMissing:'路径引用已失效，请重新添加',pathDrop:'松开后确认原路径，不复制文件',emptyDirectory:'这是主机目录引用，请通过 DSH 文件工具浏览目录。'});
Object.assign(messages.en,{settings:'Attachment settings',mode:'Conversation attachments',copyMode:'Copy files',pathMode:'Reference paths',modeHint:'Copy mode saves files on send. Path mode references originals; later edits affect what is read.',theme:'Appearance',plain:'Native · no image',official:'Official-inspired · blue',character:'DSH character · custom image',copyPath:'Copy path',pathCopied:'Path copied',reference:'Reference original path',pathHint:'Browsers do not expose absolute paths. Enter a file or directory path on the DSH host. Nothing is uploaded or copied.',pathLabel:'Absolute host path',referenceReady:'Original path',addPath:'Add path reference',saveFailed:'Could not save settings',nativeHint:'Standalone images use native DSH vision attachments. Images inside folders preserve structure and are read by path.',themeHint:'Backgrounds decorate attachment panels only. Choose no image to restore the native appearance.',hostPicker:'Choose host directory',pathMissing:'Path reference is unavailable; add it again',pathDrop:'Release to confirm the original path. No copying.',emptyDirectory:'Host directory reference. Browse its contents with DSH file tools.'});
...
export class AttachmentManager {
  ...
  async stage(sessionId,selection,target='conversation') {
    assert(target==='conversation'&&sessionId!=='@workspace-import','workspace-scope','File batches belong to conversations');
    assert(!this.destroyed && sessionId && this.isEditable(sessionId), 'locked', t('blocked'));
...
export async function reviewDialog(manager,sessionId,selection,target='conversation') {
  ...
  if(manager.preferences.mode==='path')return pathDialog(manager,sessionId);
  assert(!manager.destroyed,'disposed','Better Attach is no longer active');
  const view=dialog(t('inspect'),t('folderHint'));
  ...
  const confirm=button(t('add'),async()=>{
  ...
  const paint=()=>{
    ...
    try{
      ...
      summary.append(icon(selection.folder?'folder':'clip'),el('strong','ba-review-title',selectionLabel(reviewed)),el('span','ba-muted',`${reviewed.files.length} ${t('fileCount')} · ${formatBytes(reviewed.plan.bytes)}`));
...
export function mountToolbar(node,manager,getSession){
  ...
  render();for(const item of node.querySelectorAll('button')){item.title=item.textContent||t('settings');item.setAttribute('aria-label',item.title);}return()=>node.replaceChildren();
}
...
export function bindDrops({manager,getSession,getSidebar,getConversation,nativeImages=true}){
  // DSH's attachment component owns a document-bubble drag counter. Reset an
  // earlier drag once, then own the complete external-file lifecycle in capture.
  // This event stays in this page; it does not operate the OS or another tab.
  ...
  let scanController=null,suppressed=false,frame=null,pending=null,contentKey=null;
  const nativeDrop=nativeWorkspaceDrops(manager,path=>{hide();return addWorkspace(manager,path,{quiet:true});},error=>notice(error.message));
  const hide=()=>{overlay.hidden=true;pending=null;if(frame!==null)window.cancelAnimationFrame(frame);frame=null;};
  ...
  const show=destination=>{
    pending=destination;if(frame!==null)return;
    frame=window.requestAnimationFrame(()=>{
      frame=null;const dest=pending;pending=null;if(!dest?.node.isConnected)return;
      const r=dest.node.getBoundingClientRect();
      const geometry={left:r.left+6+'px',top:r.top+6+'px',width:Math.max(0,r.width-12)+'px',height:Math.max(0,r.height-12)+'px'};
      for(const [key,value] of Object.entries(geometry))if(overlay.style[key]!==value)overlay.style[key]=value;
      const key=dest.kind+':'+manager.preferences.mode+':'+document.documentElement.lang;
      if(contentKey!==key){overlay.replaceChildren(icon(dest.kind==='workspace'?'folder':'clip'),el('strong','',t(dest.kind==='workspace'?'dropWorkspace':'dropChat')),el('span','',t(dest.kind==='workspace'?'workspaceDropHint':manager.preferences.mode==='path'?'pathDrop':'dropSub')));contentKey=key;}
      if(overlay.hidden)overlay.hidden=false;
    });
  };
  const over=e=>{
    if(!isExternalFiles(e.dataTransfer)){hide();return;}
    e.preventDefault();e.stopImmediatePropagation();
    const dest=target(e);if(!dest||suppressed){e.dataTransfer.dropEffect='none';nativeDrop.cancelUncommitted();hide();return;}
    if(dest.kind==='conversation'&&(!getSession()||!manager.isEditable(getSession()))){hide();return;}
    e.preventDefault();e.dataTransfer.dropEffect='copy';show(dest);
    if(dest.kind==='workspace')nativeDrop.arm(e,dest.node);
    else nativeDrop.cancelUncommitted();
  };
  // Native dragover events may be hundreds of milliseconds apart, even while hovering.
  // End on actual boundary/lifecycle events; child-to-child leaves do not end a drag.
  const leave=e=>{if(isExternalFiles(e.dataTransfer)){e.preventDefault();e.stopImmediatePropagation();}if(!e.relatedTarget&&(e.target===document||e.target===document.documentElement||e.clientX<=0||e.clientY<=0||e.clientX>=window.innerWidth||e.clientY>=window.innerHeight)){nativeDrop.cancelUncommitted();hide();}};
  const drop=e=>{
    const dest=target(e);hide();if(!isExternalFiles(e.dataTransfer)){nativeDrop.cancelUncommitted();return;}
    e.preventDefault();e.stopImmediatePropagation();
    if(!dest){nativeDrop.cancelUncommitted();return;}
    if(suppressed){e.preventDefault();e.stopImmediatePropagation();suppressed=false;return;}
    if(dest.kind==='workspace'){
      e.preventDefault();e.stopImmediatePropagation();
      const count=Math.max([...(e.dataTransfer.items??[])].filter(item=>item.kind==='file').length,e.dataTransfer.files?.length??0);
      if(count>1){nativeDrop.cancel();notice(document.documentElement.lang.startsWith('zh')?'工作区一次只能拖入一个文件夹，请分别拖入。':'Add one workspace folder at a time. Drop each folder separately.');return;}
      try{const original=workspaceDrop(e.dataTransfer);if(original.path){nativeDrop.cancel();void addWorkspace(manager,original.path,{quiet:true}).catch(error=>notice(error.message));}else if(nativeDrop.active)void nativeDrop.drop(original.name);else registerExisting(manager,original.name);}catch(error){nativeDrop.cancel();notice(error.message);}return;
    }
    nativeDrop.cancel();
    ...
    if(manager.preferences.mode!=='path'&&nativeImages&&manager.onNativeImages&&!captured.hasDirectory&&captured.files.length&&captured.files.every(isRaster)){
      try{manager.onNativeImages(sessionId,captured.files);}catch(error){notice(error.message);}return;
    }
  ...
  const key=e=>{if(e.key==='Escape'){nativeDrop.cancel();if(!overlay.hidden){suppressed=true;hide();scanController?.abort();}}};
  const enter=e=>{if(!isExternalFiles(e.dataTransfer))return;e.preventDefault();e.stopImmediatePropagation();const dest=target(e);if(!suppressed&&dest?.kind==='workspace')nativeDrop.arm(e,dest.node);};
  const end=()=>{nativeDrop.cancelUncommitted();hide();suppressed=false;};
  const deactivate=()=>{nativeDrop.cancel();hide();suppressed=false;};
  const visibility=()=>{if(document.visibilityState!=='visible')deactivate();};
  document.addEventListener('dragenter',enter,true);document.addEventListener('dragleave',leave,true);document.addEventListener('dragover',over,true);document.addEventListener('drop',drop,true);document.addEventListener('keydown',key,true);document.addEventListener('dragend',end,true);window.addEventListener('blur',deactivate);window.addEventListener('pagehide',deactivate);document.addEventListener('visibilitychange',visibility);
  return()=>{nativeDrop.dispose();scanController?.abort();hide();overlay.remove();document.removeEventListener('dragenter',enter,true);document.removeEventListener('dragleave',leave,true);document.removeEventListener('dragover',over,true);document.removeEventListener('drop',drop,true);document.removeEventListener('keydown',key,true);document.removeEventListener('dragend',end,true);window.removeEventListener('blur',deactivate);window.removeEventListener('pagehide',deactivate);document.removeEventListener('visibilitychange',visibility);};
}

export async function addWorkspace(manager,path,{quiet=false}={}){
  const value=await manager.client('@workspace-import').json('/workspaces/register',{method:'POST',body:{path}});
  if(manager.destroyed)return value;
  if(quiet){notice(t('importSuccess'));return value;}
  const view=dialog(t('importSuccess'));view.body.append(el('p','ba-caption',t('workspaceLocation')),el('pre','ba-path',value.workspace.path));
  view.footer.append(button(t('close'),()=>view.d.close(),'ba-button'));
  if(manager.onOpenWorkspace)view.footer.append(button(t('openWorkspace'),()=>{Promise.resolve(manager.onOpenWorkspace(value.workspace)).then(()=>view.d.close()).catch(error=>notice(error.message));},'ba-button ba-primary','folder'));
  return value;
}
export function registerExisting(manager,droppedName='') {
  const view=dialog(t('import'),t('workspaceHint')),label=el('label','ba-caption ba-path-field',t('pathLabel')),input=el('input','ba-search'),error=el('p','ba-error');
  if(droppedName)view.body.append(el('p','ba-caption',droppedName));
  input.type='text';input.placeholder='/absolute/path/on/dsh-host';input.setAttribute('aria-label',t('pathLabel'));label.append(input);view.body.append(label,error);
  const submit=button(t('register'),async()=>{submit.disabled=true;error.textContent='';try{
    await addWorkspace(manager,input.value.trim());view.d.close();
  }catch(e){error.textContent=e.message;}finally{submit.disabled=false;}},'ba-button ba-primary','folder');
  if(manager.pickHostDirectory)view.footer.append(button(t('hostPicker'),async()=>{try{const path=await manager.pickHostDirectory();if(path&&view.d.open){input.value=path;input.focus();}}catch(e){error.textContent=e.message;}},'ba-button','folder'));
...
export function mountSettings(node,manager){
  ...
  const render=()=>{
    ...
    node.replaceChildren();const hero=el('div','ba-settings-hero');hero.append(el('span','ba-logo'),el('strong','','Better Attach'),el('p','ba-muted',t('nativeHint')));node.append(hero);
