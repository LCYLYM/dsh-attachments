...
export function bindDrops({manager,getSession,getSidebar,getConversation,nativeImages=true}){
  ...
  const show=destination=>{
    ...
    frame=window.requestAnimationFrame(()=>{
      ...
      if(contentKey!==key){overlay.replaceChildren(icon(dest.kind==='workspace'?'folder':'clip'),el('strong','',t(dest.kind==='workspace'?'dropWorkspace':'dropChat')),el('span','',t(manager.preferences.mode==='path'?'pathDrop':'dropSub')));contentKey=key;}
