...
export function bindDrops({manager,getSession,getSidebar,getConversation,nativeImages=true}){
  ...
  const over=e=>{
    if(!isExternalFiles(e.dataTransfer)){hide();return;}const dest=target(e);if(!dest||suppressed){hide();return;}
    if(dest.kind==='conversation'&&(!getSession()||!manager.isEditable(getSession()))){hide();return;}
    const items=[...(e.dataTransfer.items??[])].filter(i=>i.kind==='file');
    if(manager.preferences.mode!=='path'&&dest.kind==='conversation'&&nativeImages&&items.length&&items.every(i=>isRaster(i))){hide();return;}
  ...
  const leave=e=>{if(!e.relatedTarget&&(e.target===document||e.target===document.documentElement||e.clientX<=0||e.clientY<=0||e.clientX>=window.innerWidth||e.clientY>=window.innerHeight))hide();};
  const drop=e=>{
    ...
    if(dest.kind==='workspace'){
      ...
      try{const original=workspaceDrop(e.dataTransfer);if(original.path)void addWorkspace(manager,original.path).catch(error=>notice(error.message));else registerExisting(manager,original.name);}catch(error){notice(error.message);}return;
    }
  ...
  document.addEventListener('dragleave',leave,true);document.addEventListener('dragover',over,true);document.addEventListener('drop',drop,true);document.addEventListener('keydown',key,true);document.addEventListener('dragend',end,true);window.addEventListener('blur',end);
  return()=>{scanController?.abort();hide();overlay.remove();document.removeEventListener('dragleave',leave,true);document.removeEventListener('dragover',over,true);document.removeEventListener('drop',drop,true);document.removeEventListener('keydown',key,true);document.removeEventListener('dragend',end,true);window.removeEventListener('blur',end);};
}

export async function addWorkspace(manager,path){
...
export function registerExisting(manager,droppedName='') {
  const view=dialog(t('import'),t('workspaceHint')),label=el('label','ba-caption',t('pathLabel')),input=el('input','ba-search'),error=el('p','ba-error');
