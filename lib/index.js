...
import { NativeDropBridge } from './native-drop.js';
...
export function apply(ctx, config = {}) {
  ...
  const store = new AttachmentStore({
    stateRoot: path.join(root, 'records'),
  ...
  const nativeDrop = new NativeDropBridge();
  ctx.effect(() => { void nativeDrop.start().catch(() => nativeDrop.failed()); return () => nativeDrop.dispose(); }, 'better-attach: native workspace receiver');
  ctx.effect(() => web.register({ kind: 'prefix', path: API, handler: createHandler(store, { nativeDrop }) }), 'better-attach: HTTP route');
  ctx.effect(() => {
    // Pending-only cleanup; committed attachments remain until explicit cleanup.
