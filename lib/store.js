...
export class AttachmentStore {
  constructor({ stateRoot, resolveSession, registerWorkspace, maxActive = 16, maxReservedBytes = 4 * 1024 ** 3, ttl = 86400000 }) {
    Object.assign(this, { stateRoot: path.resolve(stateRoot), resolveSession, registerWorkspace, maxActive, maxReservedBytes, ttl });
    this.cache = new Map(); this.locks = new Map(); this.writes = new Map(); this.sealing = new Set(); this.ready = this.init();
  }
  async init() {
    for (const dir of [this.stateRoot]) {
  ...
  async get(id, sessionId) {
    ...
    assert(b.target === 'conversation', 'workspace-scope', 'File batches belong to conversations', 403);
  ...
  async sessionRoot(id) {
    ...
    assert(id !== '@workspace-import', 'workspace-scope', 'File batches belong to conversations', 403);
  ...
  async create(request) {
    ...
    assert(target === 'conversation' && sessionId !== '@workspace-import', 'workspace-scope', 'Workspaces register original host directories; file batches belong to conversations', 403);
    ...
    return this.lock('create', async () => {
      ...
      await this.loadAll();
      const active = [...this.cache.values()].filter(b => b.target === 'conversation' && ['staging', 'registering'].includes(b.status));
      ...
      const parent = await safeDirectory(cwd, '.dsh/tmp/attachments/better-attach-v2'.split('/').join(path.sep));
  ...
  async cleanAbandoned(now = Date.now()) {
    ...
    for (const b of this.cache.values()) if (b.target === 'conversation' && b.status === 'staging' && now - b.updatedAt > this.ttl && ![...this.writes.keys()].some(k => k.startsWith(b.id + ':'))) { await this.cancel(b.id, b.sessionId); count++; }
    return { count };
  }
}
