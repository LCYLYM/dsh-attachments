...
- `artifacts/current-node-tests.tap`：76 项核心、存储、HTTP、引用、UI 契约测试通过。
...
原生 Finder 跨窗口拖放已尝试，但工具操作没有取得可靠的成功读回；自动化目录 DragEvent 不代替该项。原生 DSH 文件引用与插件附件共存、移除后保留原引用的浏览器用例已通过（artifacts/foreign-reference.json）。Linux/Windows CI 已通过，Windows 桌面操作仍待验证。完整发布验收保持 BLOCKED，候选包可供本机审阅与使用。
...
GIF 按 25 fps 导出，MP4 与 GIF 均来自本次真实 DSH 网页录制。安装/卸载和原生引用共存记录沿用 rc.3 基线；本次复跑 76 项测试、两版宿主联调、9 项浏览器检查及完整模型演示。
...
[GitHub Actions](https://github.com/LCYLYM/dsh-attachments/actions/runs/34854298969) 的 Linux/Windows × Node.js 22.19.0/24 四组检查与 Linux 宿主联调通过。76 项本地测试包含 LF/CRLF 输入构建一致性；生成的客户端内容保持一致。该 CI 不替代桌面拖放验收。
