...
**未在已核查说明中看到一个与本次“对话暂存＋侧栏独立副本＋显式主机原目录注册＋导入检查＋错误恢复”完全一致的组合。** 这仅支持一个值得验证的组合定位，不能证明全球首个，也不能据此预测市场规模。
...
- 拖到侧栏创建的是副本；用户确实要操作原目录时，显式填写 DSH 主机路径。两个入口不混称。
...
建议的产品一句话：**文件夹拖进对话，是附件；拖进侧栏，是工作区。** 下面紧跟解释：侧栏默认创建独立副本，支持显式注册主机原目录。不要省掉后半句造成误解。
